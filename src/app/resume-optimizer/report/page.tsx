...
import { DownloadDropdown, DownloadOptionsModal } from '@/components/resume-optimizer/DownloadButton';
import type { ResumeAnalysisResponse, EditorState, OptimizationSuggestion } from '@/lib/types/resume-optimizer';
...
const ResumeOptimizerReportPage: NextPage = () => {
  ...
  const handleSuggestionAdopt = useCallback((suggestionId: string): void => {
    setState(prev => {
  ...
  const handleSuggestionIgnore = useCallback((suggestionId: string): void => {
    setState(prev => {
      const newAdopted = new Set(prev.adoptedSuggestions);
      const newIgnored = new Set(prev.ignoredSuggestions);
  ...
  const handleSuggestionModify = useCallback((suggestionId: string, modifiedText: string): void => {
    ...
    const modifiedSuggestions = JSON.parse(
      sessionStorage.getItem('modifiedSuggestions') || '{}'
    );
  ...
  // Handle reanalyze
  const handleReanalyze = useCallback(async (content: string): Promise<void> => {
  ...
  const handleApplySuggestionToEditor = useCallback((suggestionId: string, suggestion: OptimizationSuggestion): void => {
    try {
  ...
  const handleDownloadSuccess = useCallback((fileName: string): void => {
    addToast({
      type: 'success',
  ...
  const handleDownloadError = useCallback((error: string): void => {
    addToast({
      type: 'error',
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-6">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-stone-900 tracking-tight">Analysis Report</h1>
                  <p className="text-sm text-stone-600">Comprehensive resume optimization insights</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-6 text-sm text-stone-600">
                <div className="flex items-center space-x-2">
                  ...
                  <span>Analysis completed on {new Date(state.analysisResult.createdAt).toLocaleDateString()}</span>
                </div>
              ...
              <div className="flex items-center space-x-4">
                <div className="flex items-center space-x-2 text-sm text-stone-600">
                  <div className="px-3 py-1 rounded-full bg-stone-100 border border-stone-200">
                    <span className="font-semibold text-stone-700">{state.adoptedSuggestions.size}</span>
