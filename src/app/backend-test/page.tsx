...
import { Database, Loader2, Save, Settings, UserPlus, UserSearch } from 'lucide-react';
...
export default function BackendTestPage() {
  ...
  const handleFetchUser = async () => {
    if (!userIdToFetch) {
      toast({
        title: 'Input Error',
        description: 'Please enter a User ID to fetch.',
        variant: 'destructive',
      });
      return;
    }
  ...
  const handleCreateUser = async () => {
    ...
    try {
      ...
      toast({
        title: 'JSON Error',
        description: 'Invalid JSON for Feature Flags.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: 'User Created!',
        description: `${createdUser.name} (ID: ${createdUser._id}) has been added.`,
      });
  ...
  const handleUpdateUser = async () => {
    if (!userIdToUpdate) {
      toast({
        title: 'Input Error',
        description: 'Please enter the User ID of the user to update.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: 'JSON Error',
        description: 'Invalid JSON for Preferences.',
        variant: 'destructive',
      });
      return;
    }
  ...
  return (
    <div className="container mx-auto space-y-8 p-4 md:p-8">
      <header className="text-center">
        <Database className="mx-auto mb-2 h-12 w-12 text-primary" />
        <h1 className="font-bold text-3xl">Custom Backend Database Interaction</h1>
        <p className="text-muted-foreground">
          Test page for creating, fetching, and updating user data (including unique preferences)
          via a custom Node.js/Express/MongoDB backend.
        </p>
      </header>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Add a new user with name, email, and preferences to your MongoDB database.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <Select
              value={newTheme}
              onValueChange={(value: 'light' | 'dark') => setNewTheme(value)}
              disabled={isLoadingCreate}
            >
          ...
          {createdUserId && (
            <p className="text-green-600 text-sm">User created with ID: {createdUserId}</p>
          )}
        ...
        <CardContent className="space-y-4">
          ...
          {userData && (
            <div className="mt-4 rounded-md bg-muted p-4">
              <h3 className="mb-2 font-semibold text-lg">User Data:</h3>
              <pre className="overflow-x-auto text-sm">{JSON.stringify(userData, null, 2)}</pre>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Update preferences (theme, feature flags) for an existing user by their ID.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Input
          ...
          />
          <div className="space-y-1">
            <Label htmlFor="updatePreferences">
              Preferences (JSON format for the 'preferences' field)
            </Label>
            <Textarea
            ...
            <p className="text-muted-foreground text-xs">
              The entire 'preferences' object provided here will replace the existing one on the
              backend.
            </p>
          </div>
      ...
      <Card className="border-primary/20 bg-primary/5">
        ...
        <CardContent className="space-y-2 text-foreground/80 text-sm">
          <p>
            1. **Run Your Backend:** This frontend page expects your custom Node.js/Express backend
            (from the guide) to be running separately (e.g., on port 5000).
          </p>
          <p>
            2. **CORS:** Ensure your Express backend has CORS configured to allow requests from your
            Next.js frontend origin (e.g., `http://localhost:9002` during development). The provided
            backend example includes this.
          </p>
          <p>
            3. **MongoDB Setup:** Your MongoDB instance must be running and the `MONGODB_URI` in
            your backend's `.env` file must be correct.
          </p>
          <p>
            ...
            <ul>
              <li>
                ...
                <code>NEXT_PUBLIC_CUSTOM_BACKEND_URL=http://localhost:5000</code>
              </li>
              <li>
                Backend (<code>.env</code> in backend project): <code>MONGO_URI</code>,{' '}
                <code>BACKEND_PORT</code> (or `PORT`), <code>FRONTEND_URL</code>.
              </li>
            </ul>
          </p>
          <p>
            5. **User IDs:** After creating a user, their MongoDB `_id` will be shown. Use this ID
            for fetching or updating that specific user.
          </p>
          <p>
            6. **Error Handling & Security:** This is an illustrative example. Real applications
            require robust error handling, input validation, and security measures (like
            authentication/authorization for API endpoints).
          </p>
        </CardContent>
        <CardFooter>
          <p className="text-muted-foreground text-xs">
            This setup demonstrates how a Next.js frontend can interact with a custom backend to
