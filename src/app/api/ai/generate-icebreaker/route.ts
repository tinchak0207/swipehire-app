...
import { z } from 'zod';
import {
  type GenerateIcebreakerQuestionInput,
  type GenerateIcebreakerQuestionOutput,
  generateIcebreakerQuestion,
} from '@/ai/flows/icebreaker-generator';
  ...
  try {
    ...
    if (!parsedInput.success) {
      return NextResponse.json(
        { error: 'Invalid input parameters.', details: parsedInput.error.format() },
        { status: 400 }
      );
    }
...
export async function GET() {
  return NextResponse.json(
    { message: 'This endpoint is for POST requests to generate icebreakers.' },
    { status: 405 }
  );
}
