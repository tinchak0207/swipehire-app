...
import type { ActionStep, Goal } from '@/lib/types';
      ...
      <div className="card bg-base-100 shadow-xl">
        <div className="card-body">
          <div className="mb-4 flex items-center justify-between">
            ...
            <button onClick={() => setShowAddGoal(!showAddGoal)} className="btn btn-primary btn-sm">
          ...
          {showAddGoal && (
            <div className="space-y-4 rounded-lg bg-base-200 p-4">
              ...
              <div className="flex space-x-2">
                ...
                <button onClick={() => setShowAddGoal(false)} className="btn btn-ghost">
                  Cancel
                </button>
              </div>
            </div>
        ...
        <div className="alert alert-info">
          ...
          <div>
            ...
            <div className="text-xs">
              Start by adding your first career goal using the button above.
            </div>
          </div>
        </div>
      ) : (
        <div className="space-y-4">
          {sortedGoals.map((goal) => {
            ...
            return (
              <div
                key={goal.id}
                className={`card bg-base-100 shadow-md ${goal.completed ? 'opacity-75' : ''} ${isOverdue ? 'border-error border-l-4' : ''}`}
              >
                    ...
                    <div className="flex flex-1 items-start space-x-3">
                      ...
                      <div className="flex-1">
                        <div className="mb-2 flex items-center space-x-2">
                          ...
                          <span
                            className={`badge badge-${getPriorityColor(goal.priority)} badge-sm`}
                          >
                            {goal.priority.toUpperCase()}
                          </span>
                          {goal.category && (
                            <span className="badge badge-outline badge-sm">{goal.category}</span>
                          )}
                          {isOverdue && <span className="badge badge-error badge-sm">OVERDUE</span>}
                        </div>

                        <h4
                          className={`mb-2 font-medium ${goal.completed ? 'text-base-content/50 line-through' : 'text-base-content'}`}
                        >
                          {goal.text}
                        </h4>
                      ...
                      {goal.actionSteps.length === 0 ? (
                        ...
                        <div className="space-y-2">
                          {goal.actionSteps.map((step) => (
                            <div
                              key={step.id}
                              className="flex items-start space-x-3 rounded bg-base-200 p-3"
                            >
                              <input
                                type="checkbox"
                              ...
                              <div className="flex-1">
                                <p
                                  className={`text-sm ${step.completed ? 'text-base-content/50 line-through' : ''}`}
                                >
                                  {step.description}
                                </p>
                                <div className="mt-1 flex items-center space-x-4 text-base-content/60 text-xs">
                                  {step.dueDate && (
