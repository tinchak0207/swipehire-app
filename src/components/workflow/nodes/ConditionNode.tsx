...
const ConditionNode: React.FC<NodeProps> = ({ data }) => {
  return (
    <div className="card w-72 bg-accent text-accent-content shadow-xl">
      ...
      <Handle type="source" id="true" position={Position.Right} style={{ top: '30%' }} className="w-4 h-4" />
      <Handle type="source" id="false" position={Position.Right} style={{ top: '70%' }} className="w-4 h-4" />
    </div>
  );
};
