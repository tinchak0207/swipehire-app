...
const AnalyzeResumeNode: React.FC<NodeProps> = ({ data }) => {
  return (
    <div className="card w-72 bg-neutral text-neutral-content shadow-xl">
      ...
      <Handle type="source" id="pass" position={Position.Right} style={{ top: '30%' }} className="w-4 h-4" />
      <Handle type="source" id="fail" position={Position.Right} style={{ top: '70%' }} className="w-4 h-4" />
    </div>
  );
};
