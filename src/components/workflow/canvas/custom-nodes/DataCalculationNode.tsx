...
import { FiCpu, FiChevronDown, FiChevronUp, FiPlus, FiTrash2 } from 'react-icons/fi';
...
const DataCalculationNode: React.FC<NodeProps<DataCalculationNodeData>> = ({ data, id }) => {
  ...
  const [steps, setSteps] = useState<CalculationStep[]>(data.steps || [{ outputVariable: '', expression: '' }]);
  ...
  return (
    <div className="card w-96 bg-base-100 shadow-xl border-2 border-secondary-focus">
      <Handle type="target" position={Position.Left} id="input" className="w-4 h-4 !bg-secondary-focus" />
      <div className="card-body p-4">
        ...
        {expanded && (
          <div className="mt-4 space-y-4">
            <p className="text-sm text-gray-600">Define one or more calculations to transform data. Use variables from previous nodes, e.g., `input.score * 100`.</p>
            ...
            {steps.map((step, index) => (
              <div key={index} className="p-2 border border-base-300 rounded-lg space-y-2">
                ...
                <div className="form-control">
                  <label className="label"><span className="label-text">Output Variable Name</span></label>
                  <input type="text" placeholder="e.g., eligibilityScore" className="input input-sm input-bordered" value={step.outputVariable} onChange={(e) => updateStep(index, 'outputVariable', e.target.value)} />
                </div>
                <div className="form-control">
                  <label className="label"><span className="label-text">Expression</span></label>
                  <input type="text" placeholder="e.g., (scores.technical + scores.cultureFit) / 2" className="input input-sm input-bordered" value={step.expression} onChange={(e) => updateStep(index, 'expression', e.target.value)} />
                </div>
              </div>
            ))}
      ...
      <Handle type="source" position={Position.Right} id="output" className="w-4 h-4 !bg-secondary-focus" />
    </div>
  );
};
