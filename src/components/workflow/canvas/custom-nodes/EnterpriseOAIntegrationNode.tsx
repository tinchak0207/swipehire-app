...
const EnterpriseOAIntegrationNode: React.FC<NodeProps<EnterpriseOAIntegrationNodeData>> = ({ data, id }) => {
  ...
  return (
    <div className="card w-96 bg-base-100 shadow-xl border-2 border-gray-700">
      ...
      <div className="card-body p-4">
        ...
        {expanded && (
          <div className="mt-4 space-y-4">
            <p className="text-sm text-gray-600">Performs an action in a connected enterprise system like an HRIS or ERP.</p>
            
            <div className="form-control w-full">
              <label className="label"><span className="label-text font-semibold">System</span></label>
              <select className="select select-bordered w-full" value={system} onChange={(e) => setSystem(e.target.value as any)}>
            ...
            <div className="form-control w-full">
              <label className="label"><span className="label-text font-semibold">Action / API Endpoint</span></label>
              <input type="text" placeholder="e.g., /v1/employees or CreateNewHire" className="input input-bordered w-full" value={action} onChange={(e) => setAction(e.target.value)} />
            </div>

            <div className="form-control w-full">
              <label className="label"><span className="label-text font-semibold">Payload (JSON)</span></label>
              <textarea 
      ...
      <Handle type="source" position={Position.Right} id="output" className="w-4 h-4 !bg-gray-700" />
    </div>
  );
};
