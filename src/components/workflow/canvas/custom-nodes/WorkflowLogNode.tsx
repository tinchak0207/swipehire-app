...
const WorkflowLogNode: React.FC<NodeProps<WorkflowLogNodeData>> = ({ data }) => {
  ...
  return (
    <div className={`card w-96 bg-base-100 shadow-xl border-2 ${levelClasses[logLevel].split(' ')[0]}`}>
      <Handle type="target" position={Position.Left} id="input" className={`w-4 h-4 !${levelClasses[logLevel].split(' ')[1]}`} />
      <div className="card-body p-4">
        <div className="flex items-center justify-between cursor-pointer" onClick={() => setExpanded(!expanded)}>
          <div className="flex items-center space-x-3">
            <div className="avatar placeholder">
              <div className={`${levelClasses[logLevel].split(' ')[1]} text-white rounded-full w-12 h-12 flex items-center justify-center`}>
        ...
        {expanded && (
          <div className="mt-4 space-y-4">
            <p className="text-sm text-gray-600">Logs a message to the workflow's execution history. Useful for debugging and tracking.</p>
            
            <div className="form-control w-full">
              <label className="label"><span className="label-text font-semibold">Log Level</span></label>
              <select className="select select-bordered w-full" value={logLevel} onChange={(e) => setLogLevel(e.target.value as any)}>
            ...
            <div className="form-control w-full">
              <label className="label"><span className="label-text font-semibold">Log Message</span></label>
              <textarea 
      ...
      <Handle type="source" position={Position.Right} id="output" className={`w-4 h-4 !${levelClasses[logLevel].split(' ')[1]}`} />
    </div>
  );
};
