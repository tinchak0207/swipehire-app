...
const JobPostingNode: React.FC<NodeProps<JobPostingNodeData>> = ({ data }) => {
  ...
  const [selectedPlatforms, setSelectedPlatforms] = useState<string[]>(data?.platforms || ['LinkedIn']);
  ...
  return (
    <div className="card w-96 bg-base-100 shadow-xl border-2 border-info-focus">
      <Handle type="target" position={Position.Left} id="input" className="w-4 h-4 !bg-info-focus" />
      <div className="card-body p-4">
        ...
        {expanded && (
          <div className="mt-4 space-y-4">
            <p className="text-sm text-gray-600">This node posts a job description to selected platforms. Job data can be passed from a previous node or entered manually.</p>
            ...
            <div className="alert alert-success text-xs mt-2">
              ...
              <span>The output will contain the status of each job posting (e.g., success, failure, URL).</span>
            </div>
          </div>
        )}
      </div>
      <Handle type="source" position={Position.Right} id="output" className="w-4 h-4 !bg-info-focus" />
    </div>
  );
};
