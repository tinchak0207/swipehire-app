...
const PriorityJudgmentNode: React.FC<NodeProps<PriorityJudgmentNodeData>> = ({ data, id }) => {
  ...
  const [rules, setRules] = useState<PriorityRule[]>(data?.rules || [{ condition: '', priority: 'Medium' }]);
      ...
      <div className="card-body p-4">
        ...
        {expanded && (
          <div className="mt-4 space-y-4">
            <p className="text-sm text-gray-600">Assigns a priority level based on a set of rules. The first rule that evaluates to true determines the priority.</p>
            ...
            {rules.map((rule, index) => (
              <div key={index} className="p-2 border border-base-300 rounded-lg space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => removeRule(index)} className="btn btn-xs btn-ghost"><FiTrash2 className="text-error"/></button>
                </div>
                <div className="form-control">
                  <label className="label"><span className="label-text">Condition (e.g., data.score &gt; 80)</span></label>
                  <input type="text" placeholder="data.field === 'value'" className="input input-sm input-bordered font-mono text-xs" value={rule.condition} onChange={(e) => updateRule(index, 'condition', e.target.value)} />
                </div>
                <div className="form-control">
                  <label className="label"><span className="label-text">Set Priority to</span></label>
                  <select className="select select-sm select-bordered" value={rule.priority} onChange={(e) => updateRule(index, 'priority', e.target.value)}>
                    <option>High</option>
            ...
            <button onClick={addRule} className="btn btn-sm btn-outline btn-primary"><FiPlus /> Add Rule</button>

            <div className="form-control w-full">
              <label className="label"><span className="label-text font-semibold">Default Priority</span></label>
              <select className="select select-bordered w-full" value={defaultPriority} onChange={(e) => setDefaultPriority(e.target.value as any)}>
                <option>Low</option>
                <option>Medium</option>
                <option>High</option>
              </select>
