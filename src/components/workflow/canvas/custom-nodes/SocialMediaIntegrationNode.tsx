...
import { FiShare2, FiLinkedin, FiTwitter, FiFacebook, FiChevronDown, FiChevronUp } from 'react-icons/fi';
...
const SocialMediaIntegrationNode: React.FC<NodeProps<SocialMediaIntegrationNodeData>> = ({ data }) => {
      ...
      <div className="card-body p-4">
        ...
        {expanded && (
          <div className="mt-4 space-y-4">
            <p className="text-sm text-gray-600">Interact with social media platforms like LinkedIn, Twitter, or Facebook.</p>
            
            <div className="form-control w-full">
              <label className="label"><span className="label-text font-semibold">Platform</span></label>
              <select className="select select-bordered w-full" value={platform} onChange={(e) => setPlatform(e.target.value as any)}>
            ...
            {action === 'message' && (
              <div className="form-control w-full">
                <label className="label"><span className="label-text font-semibold">Recipient Profile URL</span></label>
                <input type="text" placeholder="e.g., https://linkedin.com/in/username" className="input input-bordered w-full" />
              </div>
            )}
            ...
            <div className="alert alert-info text-xs mt-2">
              ...
              <span>The output will contain the URL of the post or a confirmation of the message sent.</span>
            </div>
          </div>
        )}
      </div>
      <Handle type="source" position={Position.Right} id="output" className="w-4 h-4 !bg-blue-600" />
    </div>
  );
};
