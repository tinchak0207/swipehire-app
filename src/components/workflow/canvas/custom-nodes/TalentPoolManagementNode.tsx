...
import { FiDatabase, FiChevronDown, FiChevronUp, FiPlus, FiTrash2 } from 'react-icons/fi';
      ...
      <div className="card-body p-4">
        ...
        {expanded && (
          <div className="mt-4 space-y-4">
            <p className="text-sm text-gray-600">Adds, updates, or searches for candidates in your central talent pool.</p>
            
            <div className="form-control w-full">
              <label className="label"><span className="label-text font-semibold">Action</span></label>
            ...
            {action === 'add' && (
              <div className="form-control w-full">
                <label className="label"><span className="label-text font-semibold">Tags to Apply</span></label>
                <div className="flex flex-wrap gap-2 mb-2">
                  {tags.map(tag => (
                    <div key={tag} className="badge badge-lg badge-primary gap-2">
                      <button onClick={() => removeTag(tag)}><FiTrash2 size={12} /></button>
                      {tag}
                    </div>
                  ))}
                </div>
                <div className="join">
                  <input type="text" value={newTag} onChange={(e) => setNewTag(e.target.value)} placeholder="e.g., 'frontend-dev'" className="input input-bordered join-item w-full" />
                  <button onClick={addTag} className="btn join-item btn-primary"><FiPlus /></button>
                </div>
              </div>
            )}
            ...
            {action === 'search' && (
              <div className="form-control w-full">
                <label className="label"><span className="label-text font-semibold">Search Query</span></label>
                <textarea className="textarea textarea-bordered h-20" placeholder="e.g., (javascript OR python) AND (react OR vue) AND NOT angular"></textarea>
              </div>
            )}
            ...
            <div className="alert alert-lime text-xs mt-2">
              ...
              <span>For 'Add', output is the candidate record. For 'Search', output is an array of matching candidates.</span>
            </div>
          </div>
        )}
      </div>
      <Handle type="source" position={Position.Right} id="output" className="w-4 h-4 !bg-lime-500" />
    </div>
  );
};
