...
import { FiFilePlus, FiChevronDown, FiChevronUp } from 'react-icons/fi';
...
const NewResumeSubmissionTriggerNode: React.FC<NodeProps<NewResumeSubmissionTriggerNodeData>> = ({ data, id }) => {
      ...
      <div className="card-body p-4">
        ...
        {expanded && (
          <div className="mt-4 space-y-4">
            <p className="text-sm text-gray-600">Triggers the workflow when a new resume is submitted through a specific channel.</p>
            
            <div className="form-control w-full">
              <label className="label"><span className="label-text font-semibold">Submission Source</span></label>
              <select className="select select-bordered w-full" value={source} onChange={(e) => setSource(e.target.value)}>
            ...
            <div className="form-control w-full">
              <label className="label"><span className="label-text font-semibold">Filter by Job ID (Optional)</span></label>
              <input 
                type="text" 
              ...
              <label className="label">
                <span className="label-text-alt">Leave blank to trigger for any job. Use comma-separated values for multiple jobs.</span>
              </label>
            </div>

            <div className="alert alert-info text-xs mt-2">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" className="stroke-current shrink-0 w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
              <span>The output will be the submitted candidate data and the resume file object.</span>
            </div>
          </div>
        )}
      </div>
