...
const TaskAllocationNode: React.FC<NodeProps<TaskAllocationNodeData>> = ({ data }) => {
  ...
  return (
    <div className="card w-96 bg-base-100 shadow-xl border-2 border-indigo-500">
      <Handle type="target" position={Position.Left} id="input" className="w-4 h-4 !bg-indigo-500" />
      <div className="card-body p-4">
        ...
        {expanded && (
          <div className="mt-4 space-y-4">
            <p className="text-sm text-gray-600">Creates a task in your project management system and assigns it to a user or team.</p>
            
            <div className="form-control w-full">
              <label className="label"><span className="label-text font-semibold">Assign To</span></label>
              <select className="select select-bordered w-full" value={assignee} onChange={(e) => setAssignee(e.target.value)}>
            ...
            {(assignee === 'specific-user' || assignee === 'specific-team') && (
              <div className="form-control w-full">
                <label className="label"><span className="label-text font-semibold">{assignee === 'specific-user' ? 'User ID' : 'Team ID'}</span></label>
                <input type="text" placeholder={assignee === 'specific-user' ? 'e.g., user-abc-123' : 'e.g., team-reviewers'} className="input input-bordered w-full" />
              </div>
            )}

            <div className="form-control w-full">
              <label className="label"><span className="label-text font-semibold">Task Details</span></label>
              <textarea 
