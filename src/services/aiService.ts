 ...
 */
function isValidWeights(weights: any, _perspective: 'recruiter' | 'jobSeeker'): boolean {
...
Provide scores (0-100) and brief analysis. Keep responses concise.`;
    ...
    const fallbackOutput: ProfileRecommenderOutput = {
      ...
      personalityAssessment: [
        { trait: 'Professional', fit: 'positive', reason: 'Standard assessment' },
      ],
    ...
    const recruiterMatchScore = parsedResponse.weightedScores
      ? parsedResponse.weightedScores.skillsMatchScore *
          (effectiveRecruiterWeights.skillsMatchScore / 100) +
        parsedResponse.weightedScores.experienceRelevanceScore *
          (effectiveRecruiterWeights.experienceRelevanceScore / 100) +
        parsedResponse.weightedScores.cultureFitScore *
          (effectiveRecruiterWeights.cultureFitScore / 100) +
        parsedResponse.weightedScores.growthPotentialScore *
    ...
    const jobSeekerMatchScore = parsedResponse.candidateJobFitAnalysis?.weightedScoresForCandidate
      ? parsedResponse.candidateJobFitAnalysis.weightedScoresForCandidate.cultureFitScore *
          (effectiveJobSeekerWeights.cultureFitScore / 100) +
        parsedResponse.candidateJobFitAnalysis.weightedScoresForCandidate.jobRelevanceScore *
          (effectiveJobSeekerWeights.jobRelevanceScore / 100) +
        parsedResponse.candidateJobFitAnalysis.weightedScoresForCandidate.growthOpportunityScore *
          (effectiveJobSeekerWeights.growthOpportunityScore / 100) +
        parsedResponse.candidateJobFitAnalysis.weightedScoresForCandidate.jobConditionFitScore *
          (effectiveJobSeekerWeights.jobConditionFitScore / 100)
      : 50;
...
Make it authentic and engaging.`;
    ...
    const fallbackOutput = {
      script:
        "Hello, I'm excited to introduce myself and share why I'd be a great fit for this opportunity...",
      tips: [
...
Make it personalized and professional (1-2 sentences).`;
    ...
    const fallbackOutput = {
      icebreaker:
        "I noticed your background aligns well with what we're looking for. I'd love to learn more about your experience!",
      alternatives: [
