...
class AIService {
  async analyzeTextResume(resumeText: string, config: IAnalyzeResumeConfig): Promise<AnalysisResult> {
    ...
    return {
      ...
      degree: 'Bachelor',
      ai_suggestion: 'Candidate seems to have strong frontend skills. Focus on backend knowledge during the interview.',
    };
  }
  ...
  async generateInterviewQuestions(position_name: string, skills: string[], most_needed_skill: string): Promise<string[]> {
