...
export async function recordLike(payload: RecordLikePayload): Promise<RecordLikeResponse> {
  try {
    ...
    console.log(
      `[Server Action - recordLike] ENV NEXT_PUBLIC_CUSTOM_BACKEND_URL: ${CUSTOM_BACKEND_URL_FROM_ENV}`
    );
    console.log(
      `[Server Action - recordLike] Effective CUSTOM_BACKEND_URL for fetch: ${CUSTOM_BACKEND_URL}`
    );
    ...
    if (response.ok) {
      ...
      console.warn(
        '[Server Action - recordLike] Service success response was not JSON. Received text:',
        `${textResponse.substring(0, 200)}...`
      );
      throw new Error(
        `Unexpected response format from server. Status: ${response.status}. Response: ${textResponse.substring(0, 100)}...`
      );
    }
    let errorData;
    ...
    if (contentType?.includes('application/json')) {
      ...
      console.error(
        '[Server Action - recordLike] Service error (Non-JSON response). Status:',
        response.status,
        'Body:',
      ...
      const preMatch = textError.match(/<pre>([\s\S]*?)<\/pre>/i);
...
export async function fetchMatches(
  ...
  try {
    ...
    console.log(
      `[Server Action - fetchMatches] Effective CUSTOM_BACKEND_URL for fetch: ${CUSTOM_BACKEND_URL}`
    );
    ...
    if (!response.ok) {
      const errorData = await response
        .json()
        .catch(() => ({ message: `Failed to fetch matches. Status: ${response.status}` }));
      console.error('[Server Action - fetchMatches] Service error response:', errorData);
      return {
        data: null,
        error: errorData.message || `Failed to fetch matches. Status: ${response.status}`,
      };
    }
    ...
    return {
      data: null,
      error: error.message || 'An unknown error occurred while fetching matches.',
    };
  }
}
...
export async function archiveMatch(matchId: string, archivingUserId: string): Promise<Match> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        responseData.message || `Failed to archive match. Status: ${response.status}`
      );
    }
    console.log(
      '[Server Action - archiveMatch] Match archived successfully:',
      responseData.match?._id
    );
