...
export const runWorkflow = async (workflow: Workflow, initialPayload: any) => {
    ...
    const findStartNode = (): Node | undefined => nodes.find((node: Node) => node.type === 'newCandidate');
    const findNextNode = (sourceNodeId: string, sourceHandle: string | null): Node | undefined => {
        const edge = edges.find((edge: Edge) => edge.source === sourceNodeId && edge.sourceHandle === sourceHandle);
