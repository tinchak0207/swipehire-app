...
interface SubmitReviewPayload
  extends Omit<
    CompanyReview,
    'id' | 'timestamp' | 'reviewerUserId' | '_id' | 'createdAt' | 'updatedAt'
  > {
...
export async function submitCompanyReview(
  ...
  console.log(
    '[Service: submitCompanyReview] Called with userId:',
    reviewerUserId,
    'Payload:',
    payload
  );
  ...
  try {
    ...
    console.log(
      '[Service: submitCompanyReview] Review submitted successfully via backend:',
      responseData.review
    );
    return {
      success: true,
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response
        .json()
        .catch(() => ({ message: `Failed to fetch reviews. Status: ${response.status}` }));
...
export async function getCompanyReviewSummary(companyUserId: string): Promise<{
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response
        .json()
        .catch(() => ({ message: `Failed to fetch review summary. Status: ${response.status}` }));
      throw new Error(errorData.message);
    }
    return response.json();
  } catch (error) {
